...
<body class="bg-gray-900 text-white min-h-screen flex flex-col items-center justify-center p-4" data-csrf-token="<?php echo isset($csrf_token) ? htmlspecialchars($csrf_token, ENT_QUOTES, 'UTF-8') : ''; ?>">
  ...
  <div class="w-full p-6 bg-gray-800 border-t border-gray-700">
    ...
    <div class="mb-4">
        ...
        <input type="search" id="searchInput" name="searchInput"
               class="w-full bg-white/10 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-white/40 text-white"
...
<!-- Edit Song Modal -->
<div id="editSongModal" class="fixed inset-0 hidden items-center justify-center z-50 modal-overlay" role="dialog" aria-modal="true" aria-labelledby="editSongModalTitle">
  <div class="glass-effect rounded-2xl p-6 w-full max-w-md neon-shadow mx-4">
    <div class="flex justify-between items-center mb-4">
      <h3 class="text-xl font-bold" id="editSongModalTitle">Edit Song Details</h3>
    ...
    <form id="editSongForm" class="space-y-4">
      ...
      <div>
        <label for="editSongTitle" class="block mb-2 text-sm font-medium">Title</label>
        <input type="text" id="editSongTitle" name="title" required
               class="w-full bg-white/10 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-white/30 text-white"
               placeholder="Enter song title">
      </div>
      <div>
        <label for="editSongArtist" class="block mb-2 text-sm font-medium">Artist</label>
        <input type="text" id="editSongArtist" name="artist" required
               class="w-full bg-white/10 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-white/30 text-white"
               placeholder="Enter artist name">
      </div>
      <div>
        <label for="editSongLyrics" class="block mb-2 text-sm font-medium">Lyrics</label>
        <textarea id="editSongLyrics" name="lyrics" rows="4"
                  class="w-full bg-white/10 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-white/30 text-white"
                  placeholder="Enter song lyrics"></textarea>
      </div>
      <div class="flex gap-4 pt-2">
        <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 px-4 py-3 rounded-lg font-medium flex items-center justify-center" aria-label="Save changes to song metadata">
          <i class="fas fa-save mr-2" aria-hidden="true"></i>Save Changes
        </button>
        <button type="button" id="closeEditSongModalBtn" class="flex-1 bg-red-500/20 px-4 py-3 rounded-lg hover:bg-red-500/30 font-medium" aria-label="Cancel editing song">
          Cancel
        </button>
      </div>
    </form>
